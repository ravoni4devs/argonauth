...
func (store *SqliteStore) CreateDatabase(schema ...string) error {
	...
	if sql == "" {
		return fmt.Errorf("empty sql queries to run on database %s", address)
	}
	_, err := os.Stat(address)
	if os.IsNotExist(err) {
		err = store.RunSql(sql)
		if err != nil {
			return fmt.Errorf("cannot run sql on database %s: %s", address, err)
		}
		_, err = os.Stat(address)
		if err != nil {
			return fmt.Errorf("cannot create database file %s: %s", address, err)
		}
		return nil
	}
